...
class GRPOTrainer(TRLGRPOTrainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.latent_budget_penalty_weight = getattr(
            self.args,
            "latent_budget_penalty_weight",
            0.0,
        )
    ...
    def _prepare_inputs(
        ...
        with unwrap_model_for_generation(self.model, self.accelerator) as unwrapped_model:
            ...
            if len(generation_outputs) == 6:
                (
                    completion_ids,
                    old_per_token_logps,
                    cached_prompt_kv,
                    cached_prompt_hidden_states,
                    cached_expand_idx,
                    latent_budget_info,
                ) = generation_outputs
            else:
        ...
        if latent_budget_info is not None:
            local_budget_ratio = latent_budget_info["latent_budget_ratio"].to(device).float()
            gathered_budget_ratio = gather(local_budget_ratio)
            if self.latent_budget_penalty_weight:
                rewards = rewards - self.latent_budget_penalty_weight * gathered_budget_ratio
            self._metrics["active_latent_tokens"].append(
                gather(latent_budget_info["active_latent_tokens"].to(device).float()).mean().item()
            )
            self._metrics["active_latent_chunks"].append(
                gather(latent_budget_info["active_latent_chunks"].to(device).float()).mean().item()
            )
            self._metrics["latent_budget_ratio"].append(gathered_budget_ratio.mean().item())
            if self.latent_budget_penalty_weight:
                self._metrics["rewards/latent_budget_penalty"].append(
                    (-gathered_budget_ratio).mean().item()
                )
