...
from modeling.adaptive_reason import (
    AdaptiveLatentTransformerReasoningModel,
...
class CustomSFTConfig(SFTConfig):
    ...
    budget_warmup_ratio: float = 0.1
    router_tau: float = 1.0
    min_latent_chunks: int = 2
    chunk_size: int = 16
    teacher_source: str = "generations"
    teacher_selection: str = "shortest"
    teacher_max_steps: int = 24
    teacher_step_max_length: int = 128
...
def parse_training_args() -> CustomSFTConfig:
    ...
    if training_args.reasoning_net_type not in {"fixed", "adaptive_anchor"}:
        raise ValueError("--reasoning_net_type must be either 'fixed' or 'adaptive_anchor'.")
    if training_args.teacher_source != "generations":
        raise ValueError("Only --teacher_source generations is currently supported.")
...
def build_model(training_args: CustomSFTConfig, tokenizer):
    ...
    if training_args.reasoning_net_type == "adaptive_anchor":
        ...
        model = AdaptiveLatentTransformerReasoningModel(
            slow_reasoning_model=slow_reasoning_model,
            processor=tokenizer,
            reasoning_network=reasoning_network,
            anchor_loss_weight=training_args.anchor_loss_weight,
            budget_loss_weight=training_args.budget_loss_weight,
        )
    else:
    ...
    def data_collator(batch):
        ...
        if training_args.reasoning_net_type == "adaptive_anchor":
            teacher_steps = [
                build_teacher_steps(
                    item,
                    selection=training_args.teacher_selection,
                    max_steps=training_args.teacher_max_steps,
                )
                for item in batch
            ]
            flat_steps = []
            step_mask = torch.zeros(
                len(batch),
                training_args.teacher_max_steps,
                dtype=torch.long,
            )
            for batch_index, steps in enumerate(teacher_steps):
                steps = steps[: training_args.teacher_max_steps]
                step_mask[batch_index, : len(steps)] = 1
                padded_steps = steps + [""] * (training_args.teacher_max_steps - len(steps))
                flat_steps.extend(padded_steps)
            ...
            try:
                teacher_inputs = tokenizer(
                    ...
                    add_special_tokens=False,
                    truncation=True,
                    max_length=training_args.teacher_step_max_length,
                    padding="max_length",
                )
            finally:
                tokenizer.padding_side = original_padding_side

            features["teacher_input_ids"] = teacher_inputs["input_ids"].reshape(
                len(batch),
                training_args.teacher_max_steps,
                -1,
            ).long()
            features["teacher_attention_mask"] = teacher_inputs["attention_mask"].reshape(
                len(batch),
                training_args.teacher_max_steps,
                -1,
            ).long()
            features["teacher_step_mask"] = step_mask
...
class BATLRTSFTTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        if hasattr(model, "set_budget_loss_scale"):
            max_steps = max(getattr(self.state, "max_steps", 0), 1)
            warmup_steps = max(int(max_steps * getattr(self.args, "budget_warmup_ratio", 0.1)), 1)
            scale = min(1.0, float(self.state.global_step + 1) / float(warmup_steps))
            model.set_budget_loss_scale(scale)
...
def main():
    ...
    trainer_cls = BATLRTSFTTrainer if training_args.reasoning_net_type == "adaptive_anchor" else Trainer
    trainer = trainer_cls(
        model=model,
        args=training_args,
