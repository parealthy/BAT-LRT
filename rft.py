...
class CustomGRPOConfig(GRPOConfig):
    ...
    def __post_init__(self):
        ...
        if self.reasoning_net_type not in {"fixed", "adaptive_anchor"}:
            raise ValueError("reasoning_net_type must be either 'fixed' or 'adaptive_anchor'.")
...
class LatentTransformerReasoningForGRPO(LatentTransformerReasoningModel):
    ...
    def __deepcopy__(self, memo):
        ...
        copied_model.fixed_latent_chunks = self.fixed_latent_chunks
    ...
    def _run_reasoning_network(
        ...
        *,
        ...
        if getattr(self.reasoning_network, "is_adaptive_reasoning_net", False):
            output = self.reasoning_network(
                prompt_hidden_states,
                attention_mask=prompt_mask,
                return_metadata=True,
                hard=hard,
                fixed_latent_chunks=self.fixed_latent_chunks,
                trim_to_active=trim_to_active,
            )
            return output.latent_trajectory.to(self.slow_reasoning_model.dtype), output
        ...
        metadata = {
            ...
            "expected_active_tokens": torch.full(
                (latent_trajectory.size(0),),
                float(latent_trajectory.size(1)),
                device=prompt_mask.device,
                dtype=torch.float32,
            ),
        }
        return latent_trajectory, metadata
    ...
    def _build_grpo_inputs(
        ...
        **kwargs,
        ...
        latent_trajectory, latent_metadata = self._run_reasoning_network(
            prompt_hidden_states,
            prompt_mask,
            hard=False,
            trim_to_active=False,
        )
        if isinstance(latent_metadata, dict):
            latent_trajectory_mask = latent_metadata["latent_attention_mask"].to(prompt_mask.dtype)
        else:
            latent_trajectory_mask = latent_metadata.latent_attention_mask.to(prompt_mask.dtype)
    ...
    def generate(
        ...
        **kwargs,
        ...
        with torch.no_grad():
            ...
            use_hard_budget = unique_ids.size(0) == 1
            unique_latent, unique_latent_metadata = self._run_reasoning_network(
                unique_hidden_states,
                unique_mask,
                hard=use_hard_budget,
                trim_to_active=use_hard_budget,
            )
            unique_latent = unique_latent.to(unique_embeddings.dtype)
            if isinstance(unique_latent_metadata, dict):
                unique_latent_mask = unique_latent_metadata["latent_attention_mask"].to(unique_mask.dtype)
            else:
                unique_latent_mask = unique_latent_metadata.latent_attention_mask.to(unique_mask.dtype)
            ...
            unique_model_mask[:, unique_mask.size(1):] = unique_latent_mask
            ...
            if getattr(self.reasoning_network, "is_adaptive_reasoning_net", False):
                if U < B:
                    budget_active_tokens = unique_latent_metadata.active_tokens[expand_idx]
                    budget_active_chunks = unique_latent_metadata.active_chunks[expand_idx]
                else:
                    budget_active_tokens = unique_latent_metadata.active_tokens
                    budget_active_chunks = unique_latent_metadata.active_chunks
                budget_info = {
                    "active_latent_tokens": budget_active_tokens.detach(),
                    "active_latent_chunks": budget_active_chunks.detach(),
                    "latent_budget_ratio": (
                        budget_active_tokens.float()
                        / float(self.reasoning_network.latent_trajectory_length)
                    ).detach(),
                }
            else:
                budget_info = None
        ...
        if actual_length == 0:
            ...
            if return_cached_states:
                result += (cached_unique_kv, cached_unique_hidden_states, expand_idx, budget_info)
            return result if len(result) > 1 else result[0]

        if return_logps and return_cached_states:
            return completion_ids, all_logps[:, :actual_length], cached_unique_kv, cached_unique_hidden_states, expand_idx, budget_info
        elif return_logps:
            return completion_ids, all_logps[:, :actual_length]
        elif return_cached_states:
            return completion_ids, cached_unique_kv, cached_unique_hidden_states, expand_idx, budget_info
...
def build_model(training_args: CustomGRPOConfig, tokenizer):
    ...
    if training_args.reasoning_net_type == "adaptive_anchor":
        reasoning_network = AdaptiveTrajectoryAnchoredReasoningNet(
            ...
            min_chunks=training_args.min_latent_chunks,
            router_tau=training_args.router_tau,
    ...
    model.fixed_latent_chunks = training_args.fixed_latent_chunks
