...
def _project_dataset(dataset, transform_fn, keep_columns: set[str] | None = None):
    dataset = dataset.map(transform_fn, batched=False)
    keep_columns = keep_columns or {"problem", "solution"}
    columns = set(dataset.column_names) - keep_columns
    if columns:
        ...
        case "open-r1/OpenR1-Math-220k":
            ...
            return _project_dataset(
                ...
                lambda example: {
                    ...
                    "correctness_llama": example.get("correctness_llama", []),
                    "finish_reasons": example.get("finish_reasons", []),
                },
                keep_columns={
                    "problem",
                    "solution",
