...
class LatentReasoningInteractive:
    ...
    def __init__(
        ...
        fixed_latent_chunks: int | None = None,
        print_latent_budget: bool = False,
        ...
        if reasoning_net_type == "adaptive_anchor":
            self.reasoning_network = AdaptiveTrajectoryAnchoredReasoningNet(
                reasoning_net_path,
                latent_trajectory_length=latent_trajectory_length,
                hidden_size=hidden_size,
                chunk_size=chunk_size,
                min_chunks=min_latent_chunks,
                router_tau=router_tau,
            )
        elif reasoning_net_type == "fixed":
    ...
    def generate(self, user_input: str, temperature: float = 0.0) -> str:
        ...
        if getattr(self.reasoning_network, "is_adaptive_reasoning_net", False):
            ...
            latent_mask = reasoning_output.latent_attention_mask.to(attention_mask.dtype)
            budget_info = {
                "active_chunks": int(reasoning_output.active_chunks[0].item()),
                "active_tokens": int(reasoning_output.active_tokens[0].item()),
                "budget_value": float(reasoning_output.budget_values[0].item()),
            }
        else:
        ...
        if self.print_latent_budget and budget_info is not None:
            print(
                "  latent budget: "
                f"{budget_info['active_chunks']} chunks / "
                f"{budget_info['active_tokens']} tokens "
                f"(router={budget_info['budget_value']:.2f})"
            )
        return answer
...
def parse_args():
    ...
    parser.add_argument("--reasoning_net_type", type=str, default="fixed", choices=["fixed", "adaptive_anchor"])
